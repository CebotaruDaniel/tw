...
for($i = 29; $i <= 35; $i++) {
    $cantitate[$i] = mysqli_query($connection, "SELECT cantitatea FROM pret natural join tip_cereala natural join cantitate where id_cereala=4 AND id_cantitate = $i");
    $resPersonaje[$i] = $cantitate[$i]->fetch_assoc();
}

for($i = 1; $i <= 7; $i++) {
	$pret[$i] = mysqli_query($connection, "SELECT pret FROM pret natural join tip_cereala where id_cereala=4 AND id_an = $i");
	$resPret[$i] = $pret[$i]->fetch_assoc();
}
for($i = 1; $i <= 7; $i++) {
	$furnizor[$i] = mysqli_query($connection, "SELECT cantitatea FROM cantitate natural join furnizor natural join pret where id_furnizor=4 AND id_an = $i");
		...
		<script>
			...
			let myChart = new Chart(ctx, {
				...
				data: {
					...
					datasets: [{
						label: 'Cantitatea in tone a recoltei de porumb din Republica Moldova',
						data: [<?php echo $resPersonaje[35]['cantitatea'];?>, <?php echo $resPersonaje[34]['cantitatea'];?>, <?php echo $resPersonaje[33]['cantitatea'];?>, <?php echo $resPersonaje[32]['cantitatea'];?>, <?php echo $resPersonaje[31]['cantitatea'];?>, <?php echo $resPersonaje[30]['cantitatea'];?>,<?php echo $resPersonaje[29]['cantitatea'];?>],
						borderWidth: 1,
						backgroundColor: [
							'#007CC7',
							'#007CC7',
				...
				options: {
					...
					plugins: {
						title: {
							display: true,
							text: 'Cantitatea in tone a recoltei de porumb din Republica Moldova'
						}
					}
				}
			});
			...
			let myChart2 = new Chart(ctx2, {
				...
				options: {
					...
					plugins: {
						title: {
							display: true,
							text: 'Pretul în lei/kg al porumbului din Republica Moldova'
						}
					}
				}
			});
			...
			let myChart3 = new Chart(ctx3, {
				...
				data: {
					...
					datasets: [{
						label: 'Numarul de tone de porumb exportate de furnizorul RusAgro',
