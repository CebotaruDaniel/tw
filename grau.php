...
for($i = 1; $i <= 7; $i++) {
    $cantitate[$i] = mysqli_query($connection, "SELECT cantitatea FROM pret natural join tip_cereala natural join cantitate where id_cereala=1 AND id_cantitate = $i");
    $resPersonaje[$i] = $cantitate[$i]->fetch_assoc();
}

for($i = 1; $i <= 7; $i++) {
	$pret[$i] = mysqli_query($connection, "SELECT pret FROM pret natural join tip_cereala where id_cereala=1 AND id_an = $i");
	$resPret[$i] = $pret[$i]->fetch_assoc();
}

for($i = 1; $i <= 7; $i++) {
	$furnizor[$i] = mysqli_query($connection, "SELECT cantitatea FROM cantitate natural join furnizor natural join pret where id_furnizor=6 AND id_an = $i");
		...
		<script>
			...
			let myChart = new Chart(ctx, {
				...
				data: {
					...
					datasets: [{
						label: 'Cantitatea in tone a recoltei de grau din Republica Moldova',
						data: [<?php echo $resPersonaje[7]['cantitatea'];?>, <?php echo $resPersonaje[6]['cantitatea'];?>, <?php echo $resPersonaje[5]['cantitatea'];?>, <?php echo $resPersonaje[4]['cantitatea'];?>, <?php echo $resPersonaje[3]['cantitatea'];?>, <?php echo $resPersonaje[2]['cantitatea'];?>,<?php echo $resPersonaje[1]['cantitatea'];?>],
						borderWidth: 1,
						backgroundColor: [
							'#007CC7',
							'#007CC7',
				...
				options: {
					...
					plugins: {
						title: {
							display: true,
							text: 'Cantitatea in tone a recoltei de grau din Republica Moldova'
						}
					}
				}
			});
			...
			let myChart2 = new Chart(ctx2, {
				...
				options: {
					...
					plugins: {
						title: {
							display: true,
							text: 'Pretul în lei/kg al graului din Republica Moldova'
						}
					}
				}
			});
			...
			let myChart3 = new Chart(ctx3, {
				...
				data: {
					labels: ['2022', '2021','2020','2019', '2018', '2017', '2016'],
					datasets: [{
						label: 'Numarul de tone de grau exportate de furnizorul AgroNova',
						data: [<?php echo $resfurnzior[7]['cantitatea'];?>, <?php echo $resfurnzior[6]['cantitatea'];?>, <?php echo $resfurnzior[5]['cantitatea'];?>, <?php echo $resfurnzior[4]['cantitatea'];?>,<?php echo $resfurnzior[3]['cantitatea'];?>,<?php echo $resfurnzior[2]['cantitatea'];?>,<?php echo $resfurnzior[1]['cantitatea'];?>],
						borderWidth: 1,
						backgroundColor: [
							'#007CC7',
							'#4DA8DA',
