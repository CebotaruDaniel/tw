...
for($i = 8; $i <= 14; $i++) {
    $cantitate[$i] = mysqli_query($connection, "SELECT cantitatea FROM pret natural join tip_cereala natural join cantitate where id_cereala=6 AND id_cantitate = $i");
    $resPersonaje[$i] = $cantitate[$i]->fetch_assoc();
}

for($i = 1; $i <= 7; $i++) {
	$pret[$i] = mysqli_query($connection, "SELECT pret FROM pret natural join tip_cereala where id_cereala=6 AND id_an = $i");
	$resPret[$i] = $pret[$i]->fetch_assoc();
}

for($i = 1; $i <= 7; $i++) {
	$furnizor[$i] = mysqli_query($connection, "SELECT cantitatea FROM cantitate natural join furnizor natural join pret where id_furnizor=3 AND id_an = $i");
		...
		<script>
			...
			let myChart = new Chart(ctx, {
				...
				data: {
					...
					datasets: [{
						label: 'Cantitatea in tone a recoltei de hrisca din Republica Moldova',
						data: [<?php echo $resPersonaje[14]['cantitatea'];?>, <?php echo $resPersonaje[13]['cantitatea'];?>, <?php echo $resPersonaje[12]['cantitatea'];?>, <?php echo $resPersonaje[11]['cantitatea'];?>, <?php echo $resPersonaje[10]['cantitatea'];?>, <?php echo $resPersonaje[9]['cantitatea'];?>,<?php echo $resPersonaje[8]['cantitatea'];?>],
						borderWidth: 1,
						backgroundColor: [
							'#007CC7',
							'#007CC7',
				...
				options: {
					...
					plugins: {
						title: {
							display: true,
							text: 'Cantitatea in tone a recoltei de hrisca din Republica Moldova'
						}
					}
				}
			});
			...
			let myChart3 = new Chart(ctx3, {
				...
				data: {
					...
					datasets: [{
						label: 'Numarul de tone de hrisca exportate de furnizorul AgroRim',
