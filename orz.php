...
for($i = 15; $i <= 21; $i++) {
    $cantitate[$i] = mysqli_query($connection, "SELECT cantitatea FROM pret natural join tip_cereala natural join cantitate where id_cereala=2 AND id_cantitate = $i");
    $resPersonaje[$i] = $cantitate[$i]->fetch_assoc();
}

for($i = 1; $i <= 7; $i++) {
	$pret[$i] = mysqli_query($connection, "SELECT pret FROM pret natural join tip_cereala where id_cereala=2 AND id_an = $i");
	$resPret[$i] = $pret[$i]->fetch_assoc();
}

for($i = 1; $i <= 7; $i++) {
	$furnizor[$i] = mysqli_query($connection, "SELECT cantitatea FROM cantitate natural join furnizor natural join pret where id_furnizor=5 AND id_an = $i");
		...
		<script>
			...
			let myChart = new Chart(ctx, {
				...
				data: {
					...
					datasets: [{
						label: 'Cantitatea in tone a recoltei de orz din Republica Moldova',
						data: [<?php echo $resPersonaje[21]['cantitatea'];?>, <?php echo $resPersonaje[20]['cantitatea'];?>, <?php echo $resPersonaje[19]['cantitatea'];?>, <?php echo $resPersonaje[18]['cantitatea'];?>, <?php echo $resPersonaje[17]['cantitatea'];?>, <?php echo $resPersonaje[16]['cantitatea'];?>,<?php echo $resPersonaje[15]['cantitatea'];?>],
						borderWidth: 1,
						backgroundColor: [
							'#007CC7',
							'#007CC7',
				...
				options: {
					...
					plugins: {
						title: {
							display: true,
							text: 'Cantitatea in tone a recoltei de orz din Republica Moldova'
						}
					}
				}
			});
			...
			let myChart2 = new Chart(ctx2, {
				...
				options: {
					...
					plugins: {
						title: {
							display: true,
							text: 'Pretul în lei/kg al orzului din Republica Moldova'
						}
					}
				}
			});
			...
			let myChart3 = new Chart(ctx3, {
				...
				data: {
					...
					datasets: [{
						label: 'Numarul de tone de orz exportate de furnizorul BioProtect',
